...
@interface ProductOrderController (){
    
    NSInteger selectRow;
    NSInteger selectcomponent;
    NSString *countprice;
    NSString *paycountprice;
    
}

@end

@implementation ProductOrderController
@synthesize Pro_id;
...
- (void)viewDidLoad {
    ...
    [ConTitleView setFont:[UIFont fontWithName:@"ArialHebrew" size:14.0]];
    ...
    UIBarButtonItem *sLeftBarButton = [[UIBarButtonItem alloc]initWithCustomView:proLeftBack];
    
    self.navigationItem.leftBarButtonItem = sLeftBarButton;
    
    
    UIButton *RightBackPro = [UIButton buttonWithType:UIButtonTypeSystem];
    RightBackPro.titleLabel.font = [UIFont systemFontOfSize:12.0];
    RightBackPro.frame = CGRectMake(0, 0, 30, 30);
    [RightBackPro setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    RightBackPro.titleLabel.font = [UIFont fontWithName:@"iconfont" size:16];
    [RightBackPro setTitle:@"\U0000e60a" forState:UIControlStateNormal];
    [RightBackPro addTarget:self action:@selector(proinfotellConFunc:) forControlEvents:UIControlEventTouchUpInside];
    
    UIBarButtonItem *RightButtonItem = [[UIBarButtonItem alloc]initWithCustomView:RightBackPro];
    ...
    //定义pickerView
    pickerTimeview = [[PickerTimeView alloc]initWithFrame:CGRectMake(0, SIZE_HEIGHT-208, SIZE_WIDTH, pickerHeight)];
    //定义遮罩view
    backgroundview = [[UIView alloc]initWithFrame:CGRectMake(0,SIZE_HEIGHT,SIZE_WIDTH, SIZE_HEIGHT)];
    backgroundview.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.2];
    backgroundview.userInteractionEnabled = YES;
    
    //定义toolbar
    
    baseView = [[UIView alloc] initWithFrame:CGRectMake(0, backgroundview.frame.size.height-30-PICKERVIEW_HEIGHT, SIZE_WIDTH, PICKERVIEW_HEIGHT)];
    baseView.backgroundColor = [UIColor orangeColor];
    [backgroundview addSubview:baseView];
    
    UIButton *btnOK = [[UIButton alloc] initWithFrame:CGRectMake(SIZE_WIDTH-70, 0, 60,40)];
    ...
    [baseView addSubview:pickerTimeview];
...
-(void)loadDatapicker{
    ...
    for (int i=0; i<letter.count; i++) {
        ...
        [number addObject:[letter[i] valueForKey:@"hour"]];
        
    }
    
    pickerTimeview.letter = letter;
    pickerTimeview.number = number;
...
-(void)TabViewFunc{
    ...
    [TabLeView setSeparatorColor:[UIColor colorWithRed:227/255.0 green:227/255.0 blue:227/255.0 alpha:0.8]];
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    return 11+arrayMutableProduct.count;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0) {
        ...
        addressLabel_title = [[UILabel alloc]initWithFrame:CGRectMake(SIZE_WIDTH-(SIZE_WIDTH/2)-30, 0, SIZE_WIDTH/2, 40)];
        addressLabel_title.text =yuyuestr;
        addressLabel_title.textAlignment = NSTextAlignmentCenter;
        addressLabel_title.font = [UIFont systemFontOfSize:12.0];
        addressLabel_title.textColor = [UIColor colorWithRed:96/255.0 green:96/255.0 blue:96/255.0 alpha:0.8];
        addressLabel_title.userInteractionEnabled = YES;
        addressLabel_title.enabled = YES;
        
        [OneView addSubview:addressLabel_title];
        
        UITapGestureRecognizer *taptouchAddress = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(touchtime:)];
        [addressLabel_title addGestureRecognizer:taptouchAddress];
        ...
        CellBackView.backgroundColor = [UIColor colorWithRed:232/255.0 green:232/255.0 blue:232/255.0 alpha:1.0];
        
        UILabel *LabelName = [[UILabel alloc]initWithFrame:CGRectMake(10, 0, SIZE_WIDTH/3,30)];
        LabelName.text = @"预约项目";
        ...
        [CellBackView addSubview:LabelName];
        ...
        if ([[[arrayMutableProduct objectAtIndex:0] valueForKey:@"type"] intValue]== 1) {
            ...
            //    UIView *cellBackGround = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SIZE_WIDTH, 40)];
            //    cellBackGround.backgroundColor = [UIColor colorWithRed:232/255.0 green:232/255.0 blue:232/255.0 alpha:0.4];
            //                UILabel *empLabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 10, SIZE_WIDTH-20, 20)];
            //                empLabel.text = @"该产品没有标签别的规格可以选择!";
            //                empLabel.font = [UIFont systemFontOfSize:10.0];
            //                empLabel.textColor = [UIColor colorWithRed:96/255.0 green:96/255.0 blue:96/255.0 alpha:0.6];
            //                [cellBackGround addSubview:empLabel];
            
        }else{
            
            UIView *cellBackGround = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SIZE_WIDTH, 40)];
            cellBackGround.backgroundColor = [UIColor colorWithRed:232/255.0 green:232/255.0 blue:232/255.0 alpha:0.4];
            emapArray =[[arrayMutableProduct objectAtIndex:0] valueForKey:@"emap"];
            double (^sumBlock)(int) =^(int a){
            ...
            int leftWidth = 0;
            for(int i=0;i < emapArray.count;i++){
                
                UIButton *but=[UIButton buttonWithType:UIButtonTypeRoundedRect];
                [but setTitle:[[emapArray objectAtIndex:i] valueForKey:@"pro_color"] forState:UIControlStateNormal];
                but.titleLabel.textAlignment = NSTextAlignmentCenter;
                but.titleLabel.font = [UIFont systemFontOfSize:12.0];
                [but setBackgroundColor:[UIColor whiteColor]];
                but.layer.borderWidth = 1;
                
                [but addTarget:self action:@selector(BQEventFunc:) forControlEvents:UIControlEventTouchUpInside];
                but.tag = i ;
                
                NSString *eid = [[arrayMutableProduct objectAtIndex:0] valueForKey:@"eid"];
                if ([eid isEqualToString:[[emapArray objectAtIndex:i] valueForKey:@"id"]]) {
                ...
                int isNum =(int)[[[emapArray objectAtIndex:i] valueForKey:@"pro_color"] length];
                
                double widthNum =sumBlock(isNum);
                widthNum +=15;
                if(i==0) leftWidth = 5;
                
                but.frame = CGRectMake(leftWidth+5*i,5,widthNum , 25);
                
                leftWidth += widthNum;
                
                [cellBackGround addSubview:but];
                
                [arrayBut addObject:but];
        ...
        UILabel *BZLable = [[UILabel alloc]initWithFrame:CGRectMake(10, 5, 120, 30)];
        BZLable.text = @"优惠券";
        BZLable.textColor = [UIColor colorWithRed:96/255.0 green:96/255.0 blue:96/255.0 alpha:1.0];
        ...
        UILabel *yhlabel = [[UILabel alloc]initWithFrame:CGRectMake(SIZE_WIDTH-150, 5, 140, 30)];
        yhlabel.text = @"暂无可用";
        yhlabel.textAlignment = NSTextAlignmentRight;
        yhlabel.textColor = [UIColor colorWithRed:96/255.0 green:96/255.0 blue:96/255.0 alpha:0.7];
        yhlabel.font = [UIFont systemFontOfSize:14.0];
        
        
        [cell.contentView addSubview:yhlabel];
        ...
        UILabel *SgNum = [[UILabel alloc]initWithFrame:CGRectMake(10, 5, SIZE_WIDTH/4, 30)];
        SgNum.text = @"实付金额 :";
        SgNum.textColor = [UIColor colorWithRed:96/255.0 green:96/255.0 blue:96/255.0 alpha:0.7];
        SgNum.font = [UIFont systemFontOfSize:12.0];
        
        
        
        SFPricNum = [[UILabel alloc]initWithFrame:CGRectMake(SIZE_WIDTH-SIZE_WIDTH/2 -10, 5, SIZE_WIDTH/2, 30)];
        SFPricNum.text =[NSString stringWithFormat:@"¥%@",paycountprice];
        SFPricNum.textAlignment = NSTextAlignmentRight;
        ...
        SelectButWX.frame = CGRectMake(SIZE_WIDTH-40, 10, 25, 25);
        SelectButWX.tag = 1;
        if(pay_Type ==1){
        ...
        [SelectButWX addTarget:self action:@selector(selectPayBut:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [cell.contentView addSubview:ScureesView];
            ...
            UIView *ScureesView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SIZE_WIDTH, 50)];
            ScureesView.backgroundColor = [UIColor whiteColor];
            ScureesView.userInteractionEnabled = YES;
            UIImageView *imageWX = [[UIImageView alloc]initWithFrame:CGRectMake(10, 10, 30, 30)];
            ...
            SelectButZFB.frame = CGRectMake(SIZE_WIDTH-40, 10, 25, 25);
            SelectButZFB.tag = 2;
        ...
        [SelectButZFB addTarget:self action:@selector(selectPayBut:) forControlEvents:UIControlEventTouchUpInside];
        ...
        UIView *ScureesView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SIZE_WIDTH, 60)];
        ScureesView.backgroundColor = [UIColor whiteColor];
        ...
        NowLaBle.font = [UIFont systemFontOfSize:12.0];
        ...
        UILabel *DelegateText = [[UILabel alloc]initWithFrame:CGRectMake(30, 30, SIZE_WIDTH-70, 20)];
        DelegateText.userInteractionEnabled = YES;
        DelegateText.text = @"我已阅读并同意《时光服务平台协议》";
        DelegateText.textColor = [UIColor blackColor];
        DelegateText.font = [UIFont systemFontOfSize:12.0];
        ...
        UIView *ProductView =  [[UIView alloc]initWithFrame:CGRectMake(0, 0, SIZE_WIDTH, 295)];
        
        //产品图片
        NSString *proImageUrl = [NSString stringWithFormat:@"%@%@small.jpg",[USER_DEFAULT objectForKey:@"SERVERIP"],[[arrayMutableProduct objectAtIndex:0] valueForKey:@"main_img"]];
        ...
        asynImageView.imageURL=[NSString stringWithFormat:@"%@",[[arrayMutableProduct objectAtIndex:0] valueForKey:@"main_img"]];
        ...
        //产品名称
        UILabel *ProDuctName = [[UILabel alloc]initWithFrame:CGRectMake(10,195, SIZE_WIDTH-20, 30)];
        ProDuctName.text = [NSString stringWithFormat:@"%@",[[arrayMutableProduct objectAtIndex:0] valueForKey:@"title"]];
        ...
        //＋按钮
        UIButton *addButs = [UIButton buttonWithType:UIButtonTypeRoundedRect];
        [addButs addTarget:self action:@selector(proaddreduceFunc:) forControlEvents:UIControlEventTouchUpInside];
        addButs.frame = CGRectMake(120,145+80+5, 25, 25);
        addButs.tag = 1;
        [addButs setTitle:@"+" forState:UIControlStateNormal];
        [addButs setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        addButs.titleLabel.font = [UIFont systemFontOfSize:16.0];
        addButs.backgroundColor = [UIColor colorWithRed:239/255.0 green:127/255.0 blue:52/255.0 alpha:1.0];
        [addButs.layer setMasksToBounds:YES];
        [addButs.layer setCornerRadius:3.0];
        [ProductView addSubview:addButs];
        
        //-按钮
        UIButton *reduceBut = [UIButton buttonWithType:UIButtonTypeRoundedRect];
        [reduceBut addTarget:self action:@selector(proaddreduceFunc:) forControlEvents:UIControlEventTouchUpInside];
        reduceBut.frame = CGRectMake(20,145+80+5 , 25, 25);
        reduceBut.backgroundColor = [UIColor colorWithRed:239/255.0 green:127/255.0 blue:52/255.0 alpha:1.0];
        [reduceBut setTitle:@"-" forState:UIControlStateNormal];
        [reduceBut setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        reduceBut.titleLabel.font = [UIFont systemFontOfSize:16.0];
        reduceBut.tag = 2;
        [reduceBut.layer setMasksToBounds:YES];
        [reduceBut.layer setCornerRadius:3.0];
        [ProductView addSubview:reduceBut];
        
        //产品数量
        ProNum = [[UILabel alloc]initWithFrame:CGRectMake(reduceBut.frame.size.width+25, 145+80+3,70, 25)];
        ProNum.text = [NSString stringWithFormat:@"数量 :%ld",(long)payNum];
        ProNum.font = [UIFont systemFontOfSize:14.0];
        ProNum.textColor = [UIColor colorWithRed:96/255.0 green:96/255.0 blue:96/255.0 alpha:0.5];
        
        [ProductView addSubview:ProNum];
        
        //产品价格
        
        ProPrice = [[UILabel alloc]initWithFrame:CGRectMake(20, 145+ProDuctName.frame.size.height+60+25, SIZE_WIDTH-40, 25)];
        ProPrice.text = [NSString stringWithFormat:@"价格 : ¥%@",[[arrayMutableProduct objectAtIndex:0] valueForKey:@"price"]];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 3+arrayCount+7 ) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CouponController *couponCon = NULL;
    ...
    if (arrayMutableProduct.count+5 == indexPath.row) {
        
        NSLog(@"%ld",(long)indexPath.row);
        NSLog(@"优惠券");
        
        
        couponCon=[[CouponController alloc]init];
        couponCon.pro_id = Pro_id;
        UINavigationController *counponNav=[[UINavigationController alloc]initWithRootViewController:couponCon];
        [self presentViewController:counponNav animated:YES completion:^{
...
-(void)GetHttpsDataProductslist{
    ...
    NSString *StrUrl = [NSString stringWithFormat:@"%@id=%@",strUrl,Pro_id];
    ...
    [manager GET:encoded parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        arrayMutableProduct  = responseObject;
        
        NSLog(@"arrayMutableProduct%@",arrayMutableProduct);
        
        float (^getPricNum)(NSMutableArray *)=^(NSMutableArray *array){
            
            float prinum =0;
            for (int xs = 0 ; xs<array.count; xs++) {
                
                prinum +=[NSString stringWithFormat:@"%@",[[array objectAtIndex:xs] valueForKey:@"price"]].floatValue;
                
            }
            return prinum;
            
        };
        countprice = [NSString stringWithFormat:@"¥%.2lf",getPricNum(arrayMutableProduct)];
        paycountprice= [NSString stringWithFormat:@"%.2lf",getPricNum(arrayMutableProduct)];
...
-(void)ProOriderFunc{
    ...
    UIButton *_OrderButsPro = [UIButton buttonWithType:UIButtonTypeCustom];
    _OrderButsPro.frame = CGRectMake(0, SIZE_HEIGHT-50, SIZE_WIDTH, 50);
    [_OrderButsPro setTitle:@" 立即支付 " forState:UIControlStateNormal];
    [_OrderButsPro setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_OrderButsPro setBackgroundImage:[UIImage imageNamed:@"sms_but.png"] forState:UIControlStateNormal];
    [_OrderButsPro setBackgroundImage:[UIImage imageNamed:@"sms_but.png"] forState:UIControlStateHighlighted];
    [_OrderButsPro addTarget:self action:@selector(OrderProductFuncPro:) forControlEvents:UIControlEventTouchUpInside];
    
    [self.view addSubview:_OrderButsPro];
...
#pragma mark -下单支付事件
-(void)OrderProductFuncPro:(id)sender{
    
    
    if (addressList.count==0) {
        
        UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:@"提示" message:@"请输入地址!" preferredStyle:UIAlertControllerStyleAlert];
    ...
    if (addressLabel_title.text.length<=8) {
...
//        UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:@"提示" message:@"支付宝暂未开放!" preferredStyle:UIAlertControllerStyleAlert];
//        [alertCon addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
//            
//        }]];
//        
//        [self presentViewController:alertCon animated:YES completion:nil];
        [self AlipayButtonAction];
       // [self doAlipayPay];
    }
}
...
//选中商品调用支付宝极简支付
//
- (void)doAlipayPay:(NSString *)alipayorder
{
    //重要说明
    ...
    order.biz_content.body = [NSString stringWithFormat:@"%@",[[arrayMutableProduct objectAtIndex:0] valueForKey:@"title"]];
    order.biz_content.subject = [NSString stringWithFormat:@"%@",[[arrayMutableProduct objectAtIndex:0] valueForKey:@"title"]];
    order.biz_content.out_trade_no = [self generateTradeNO]; //订单ID（由商家自行制定）
    order.biz_content.timeout_express = @"30m"; //超时时间设置
    NSString *paynumstr = paycountprice;
...
-(void)AlipayButtonAction{
    ...
    @try {
        ...
        NSString *orderDeflut = Alipay_orderUrl;
        NSString *strText = [addressLabel_title.text stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
        NSString *aid = [[addressList objectAtIndex:0] valueForKey:@"id"];
        NSString *paynumstr = [NSString stringWithFormat:@"%ld",(long)payNum];
        NSString *order_uRL = [NSString stringWithFormat:@"%@%@id=%@&price=%@&userid=%@&paytype=22&num=%@&needtime=%@&aid=%@",[USER_DEFAULT objectForKey:@"SERVERIP"],orderDeflut,Pro_id,paycountprice,[USER_DEFAULT objectForKey:@"user_id"],paynumstr,strText,aid];
        NSLog(@"order_uRL----%@",order_uRL);
        ...
        //发送请求
        [manager GET:order_uRL parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            alipayorder = [[responseObject objectAtIndex:0] valueForKey:@"oid"];
            NSLog(@"responseObject====%@",alipayorder);
            NSString *stralipays=[NSString stringWithFormat:@"%@",alipayorder];

           
            if (![stralipays isEqualToString:@""]) {
                
                [self doAlipayPay:alipayorder];
            ...
            UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:@"提示" message:@"支付失败!请稍后再试" preferredStyle:UIAlertControllerStyleAlert];
...
-(void)weChatPayButtonAction{
    ...
    //先调用统一下单接口
    //http://118.180.7.92:8088/order/iosorder.do?id=160725190415543092009&price=1&userid=admin&paytype=1&num=1&needtime=2016-12-05
    
    @try {
        
        NSString *orderDeflut = WX_orderUrl;
        NSString *strText = [addressLabel_title.text stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
        NSString *aid = [[addressList objectAtIndex:0] valueForKey:@"id"];
        NSString *paynumstr = [NSString stringWithFormat:@"%ld",(long)payNum];
        NSString *order_uRL = [NSString stringWithFormat:@"%@%@id=%@&price=%@&userid=%@&paytype=11&num=%@&needtime=%@&aid=%@",[USER_DEFAULT objectForKey:@"SERVERIP"],orderDeflut,Pro_id,paycountprice,[USER_DEFAULT objectForKey:@"user_id"],paynumstr,strText,aid];
        NSLog(@"order_uRL----%@",order_uRL);
        ...
        [manager GET:order_uRL parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            NSString *appid = @"wxd930ea5d5a258f4f";
            NSString *prepayIdrequest  = [[responseObject objectAtIndex:0] valueForKey:@"prepay_id"];
            ...
            NSString *nonceStrstr =[self generateTradeNO];
            NSDictionary *dict = @{@"appid":appid,@"nonceStr":nonceStrstr,@"package":@"Sign=WXPay",@"partnerId":@"1407117902",@"prepayId":prepayIdrequest,@"timeStamps":timestamp};
            ...
            for (NSString *str in sortedArray) {
                ...
                if (![[dict objectForKey:str] isEqualToString:@""] && ![str isEqualToString:@"sign"] && ![str isEqualToString:@"key"]) {
                    
                    [contentString appendFormat:@"%@=%@&",str,dict[str]];
                    
                }
                
            }
            orderNum = [[responseObject objectAtIndex:0] valueForKey:@"oid"];
...
-(void)TouchDelegateFunc:(id)sender{
    ...
    DeleteController *deleteCons = [[DeleteController alloc]init];
    deleteCons.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:deleteCons animated:YES];
...
#pragma mark-数量添加按钮
-(void)proaddreduceFunc:(UIButton *)but{

    if (but.tag == 1) {
        
        
        if (payNum>=1 && payNum<=99){
            ...
            if ([[[arrayMutableProduct objectAtIndex:0] valueForKey:@"type"] intValue]== 1) {
                ...
                pricestr = [[arrayMutableProduct objectAtIndex:0] valueForKey:@"price"];
                
            }else{
                
                pricestr = [[emapArray objectAtIndex:bqNumindex] valueForKey:@"price"];
            ...
            countprice = [NSString stringWithFormat:@"¥%.2lf",PRICE];
            paycountprice = [NSString stringWithFormat:@"%.2lf",PRICE];
            HJPricNum.text =countprice;
            SFPricNum.text =countprice;
...
#pragma mark -拨打电话方法
-(void)proinfotellConFunc:(id)sender{
...
-(void)getOrderPayResult:(NSNotification *)notification{
    ...
    orderMessageCon.dictProduct = [arrayMutableProduct objectAtIndex:0];
    orderMessageCon.ordernum = orderNum;
...
-(void)changeOrderstatusFunc{
    ...
    NSDictionary *parameters=@{@"id":alipayorder};
    ...
    [manager POST:orderurl parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        OrderMessageController *orderMessageCon = [[OrderMessageController alloc]init];
        orderMessageCon.dictProduct = [arrayMutableProduct objectAtIndex:0];
        orderMessageCon.ordernum = alipayorder;
        [self.navigationController pushViewController:orderMessageCon animated:NO];
...
-(void)getUserCouponFunc{
    ...
    NSDictionary *parameters=@{@"user_id":[USER_DEFAULT objectForKey:@"user_id"],@"pid":Pro_id};
